...
"[-c X] [-w 80] [--usage] [--version] [--help] \x95\x5c示する文字列\n"

#define HELP_STR "Usage: %s \x95\x5c示する文字列\n\n"\
"全体的なオプション\n"\
"  -c, --char=%s       使用する文字列 <%s>\n"\
"  -w, --width=%d     \x95\x5c示幅を設定します。<%d>\n\n"\
"ヘルプオプション\n"\
"  -?, --help         このヘルプメッセージを\x95\x5c示します。\n"\
"  --usage            簡潔な使用方法を\x95\x5c示します。\n"\
"  --version          ヴァージョン情報を\x95\x5c示します。\n\n"\
"特殊オプション(WIN32)\n"\
"  -F, --fontname=\"%s\"\n"\
"                     使用フォント名を指定します。<%s>\n"\
...
"もりですが、\x83\x5cースは一切参照していませんので、実装手段は全く異なっ"\
"てい\n"\
...
{
    fprintf(stderr, "%s: 引数が不\x8f\x5c分です。\n",
...
int main(int argc, char *argv[])
{
    ...
    for (i = 1; i < argc; i++) {
        ...
        if (IS_ARG_OK2("-?", "--help")) {
            ...
            if (0 >= nScreenWidth) {
                fprintf(stderr, "%s: \x95\x5c示幅が狭すぎます。<%d>\n",
                        GetLongBaseName(argv[0]), nScreenWidth);
                return 1;
            }
            if (MAX_WIDTH <= nScreenWidth) {
                fprintf(stderr, "%s: \x95\x5c示幅が大きすぎます。<%d>\n",
                        GetLongBaseName(argv[0]), nScreenWidth);
                return 1;
            }
        }
    ...
    if (strlen(lpszUseChar) >= (ULONG)nScreenWidth) {
            fprintf(stderr, "%s: \x95\x5c示幅より、"
                    "使用文字列長が長い為\x95\x5c示できません\n",
                    GetLongBaseName(argv[0]));
        return 1;
    }

    Syslogger(FALSE, "DBG: 使用文字=%s, \x95\x5c示幅=%d, "
              "フォント=%s, フォントサイズ=%d",
